...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), has_relay=False, car_fw=[]):
    ...
    if candidate in [CAR.SANTA_FE, CAR.SANTA_FE_1]:
      ...
      ret.wheelbase = 2.7
      ret.steerRatio = 13.5
      tire_stiffness_factor = 0.685    # stiffnessFactor settled on 1.0081302973865127
      ret.lateralTuning.pid.kiBP, ret.lateralTuning.pid.kpBP = [[0.], [0.]]
...
#      events.append(create_event('steerTempUnavailable', [ET.NO_ENTRY, ET.WARNING]))
    ...
    if ret.rightBlinker and ret.rightBlindspot and ret.vEgo > (35 * CV.MPH_TO_MS):
      events.append(create_event('rightLCAbsm', [ET.WARNING]))
    if ret.leftBlinker and ret.leftBlindspot and ret.vEgo > (35 * CV.MPH_TO_MS):
